    ...
    private static class PopulateDbAysncTask extends AsyncTask<Void, Void, Void>{
        ...
        protected Void doInBackground(Void... voids) {
            try {
                ...
                ViolationCategory violationCategory = ViolationCategory(catergory, codeNum);
                String catergory = violationCategory.fromCodeNumber(codeNums).toString();

                //unable to solve this for now
                violationDao.insert(new Violation(codeNums, isCriticals, catergory, description));
